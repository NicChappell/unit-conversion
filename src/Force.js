...
class Force extends React.Component {
  ...
  handleFocusCalculator(event) {
    ...
    this.props.onFocusApp(data.force.calculator_name);
  }
  ...
  handleChangeCalculator(event) {
    ...
    this.setState({
      ...
      n: roundToThousandths(data.force.units[name].conversions['n'].calculation(value)),
      lbf: roundToThousandths(data.force.units[name].conversions['lbf'].calculation(value))
    });
  }

  render() {
    const calculator_icon = data.force.calculator_icon;
    const calculator_name = data.force.calculator_name;
    const focus = this.state.focus;
    const onFocusCalculator = this.handleFocusCalculator;
    const onChangeCalculator = this.handleChangeCalculator;
    const plural_form = data.force.plural_form;
    const showCalculator = this.props.showCalculator;
    const showCalculatorDetails = this.props.showCalculatorDetails;
    const units = data.force.units;
