...
class Area extends React.Component {
  ...
  handleFocusCalculator(event) {
    ...
    this.props.onFocusApp(data.area.calculator_name);
  }
  ...
  handleChangeCalculator(event) {
    ...
    this.setState({
      ...
      km2: roundToThousandths(data.area.units[name].conversions['km2'].calculation(value)),
      m2: roundToThousandths(data.area.units[name].conversions['m2'].calculation(value)),
      mi2: roundToThousandths(data.area.units[name].conversions['mi2'].calculation(value)),
      ft2: roundToThousandths(data.area.units[name].conversions['ft2'].calculation(value)),
      ha: roundToThousandths(data.area.units[name].conversions['ha'].calculation(value)),
      ac: roundToThousandths(data.area.units[name].conversions['ac'].calculation(value))
    });
  }

  render() {
    const calculator_icon = data.area.calculator_icon;
    const calculator_name = data.area.calculator_name;
    const focus = this.state.focus;
    const onFocusCalculator = this.handleFocusCalculator;
    const onChangeCalculator = this.handleChangeCalculator;
    const plural_form = data.area.plural_form;
    const showCalculator = this.props.showCalculator;
    const showCalculatorDetails = this.props.showCalculatorDetails;
    const units = data.area.units;
