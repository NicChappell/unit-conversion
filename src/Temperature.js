...
class Temperature extends React.Component {
  ...
  handleFocusCalculator(event) {
    ...
    this.props.onFocusApp(data.temperature.calculator_name);
  }
  ...
  handleChangeCalculator(event) {
    ...
    this.setState({
      ...
      c: roundToThousandths(data.temperature.units[name].conversions['c'].calculation(value)),
      f: roundToThousandths(data.temperature.units[name].conversions['f'].calculation(value)),
      k: roundToThousandths(data.temperature.units[name].conversions['k'].calculation(value))
    });
  }

  render() {
    const calculator_icon = data.temperature.calculator_icon;
    const calculator_name = data.temperature.calculator_name;
    const focus = this.state.focus;
    const onFocusCalculator = this.handleFocusCalculator;
    const onChangeCalculator = this.handleChangeCalculator;
    const plural_form = data.temperature.plural_form;
    const showCalculator = this.props.showCalculator;
    const showCalculatorDetails = this.props.showCalculatorDetails;
    const units = data.temperature.units;
