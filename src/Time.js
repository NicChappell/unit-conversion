...
class Time extends React.Component {
  ...
  handleFocusCalculator(event) {
    ...
    this.props.onFocusApp(data.time.calculator_name);
  }
  ...
  handleChangeCalculator(event) {
    ...
    this.setState({
      ...
      s: roundToThousandths(data.time.units[name].conversions['s'].calculation(value)),
      m: roundToThousandths(data.time.units[name].conversions['m'].calculation(value)),
      h: roundToThousandths(data.time.units[name].conversions['h'].calculation(value)),
      d: roundToThousandths(data.time.units[name].conversions['d'].calculation(value))
    });
  }

  render() {
    const calculator_icon = data.time.calculator_icon;
    const calculator_name = data.time.calculator_name;
    const focus = this.state.focus;
    const onFocusCalculator = this.handleFocusCalculator;
    const onChangeCalculator = this.handleChangeCalculator;
    const plural_form = data.time.plural_form;
    const showCalculator = this.props.showCalculator;
    const showCalculatorDetails = this.props.showCalculatorDetails;
    const units = data.time.units;
